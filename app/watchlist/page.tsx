...
function WatchlistContent() {
  const [tradingSegments, setTradingSegments] = useState<TradingSegment[]>(() => {
    if (typeof window !== 'undefined') {
      try {
        if ((window as any).__initialTradingSegments) return (window as any).__initialTradingSegments;
        const cached = localStorage.getItem('cached_trading_segments');
        if (cached) return JSON.parse(cached);
      } catch (_) {}
    }
    return [];
  });
  const tradingSegmentsRef = useRef<TradingSegment[]>(tradingSegments);
      ...
      api.get<{ segments: TradingSegment[] }>(`/api/market/instruments/library?${qs.toString()}`)
        .then(data => {
          if (data.segments) {
            ...
            try {
              localStorage.setItem('cached_trading_segments', JSON.stringify(data.segments));
            } catch (_) {}
          }
        })
  ...
  // Declarative toast auto-dismiss: automatically dismiss toast after 1s (or 2.5s for errors)
  useEffect(() => {
  ...
  const showToast = useCallback((msg: string, isError = false) => {
    setToast({ msg, isError: !!isError, visible: true });
  }, []);
