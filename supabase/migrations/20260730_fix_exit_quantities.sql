-- Description: Fix exit loop variables in place_order function.
-- Change v_remaining_qty and v_close_qty from integer to numeric to support decimal quantities (like 0.1, 0.4 ETH).
...
DECLARE
  ...
  -- Loop exit vars (Changed to NUMERIC to support decimals/cryptos)
...
BEGIN
  ...
  IF v_order.is_exit THEN
    -- LOOP THROUGH OPEN POSITIONS TO EXIT THEM
